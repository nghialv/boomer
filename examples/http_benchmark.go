...
func worker() {
	...
	if err != nil {
		...
		b.Report("request_failure", "http", "error", 0.0, err.Error())
	} else {
		if response.StatusCode == http.StatusOK {
			b.Report("request_success", "http", "200", elapsed, response.ContentLength)
		} else {
			b.Report("request_success", "http", strconv.Itoa(response.StatusCode), elapsed, response.ContentLength)
		}
		if verbose {
